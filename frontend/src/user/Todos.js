      ...
      <div className="Todos-wrapper">
        {essentialTodos.length ? (
          <div className="Todos-wrapper essential-wrapper">
            <h4>Essential</h4>
            {essentialTodos.map(t => (
              <Link className="Todos-todo essential" to={(t === 'Add a reptile') ? `/users/${currUser.id}/reptiles/add` : '/foods'} >{t}</Link>
              ))}
          </div>
        ) : null}
        {niceToHaveTodos.length ? (
          <div className="Todos-wrapper niceToHave-wrapper">
            <h4>Nice to Have</h4>
            {niceToHaveTodos.map(t => (
              <Link className="Todos-todo niceToHave" to="/foods" >{t}</Link>
            ))}
          </div>
        ) : null}
      </div>
